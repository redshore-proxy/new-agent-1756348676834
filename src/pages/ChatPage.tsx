...
const ChatPage: React.FC = () => {
  ...
  const { genAI, model, apiKeyError } = useMemo(() => {
    const API_KEY = import.meta.env.VITE_GEMINI_API_KEY; if (!API_KEY) {
  ...
  const handleSendMessage = async (text: string) => {
    if (apiKeyError || !model) {
      ...
      setMessages((prevMessages) => [...prevMessages, errorMessage]); return;
    }
    ...
    try {
      ...
      let errorMessageText = "Oops! Something went wrong with the AI. Please try again."; if (error instanceof Error) {
